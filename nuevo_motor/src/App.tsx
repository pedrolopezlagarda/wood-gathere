...
export default function App() {
  ...
  const [tileMetadata, setTileMetadata] = useState<Record<string, { priority: number; role?: TileRole; frameCount?: number; hasShadow?: boolean; sway?: boolean }>>({});
    ...
    const drawDynamicShadow = (ctx: CanvasRenderingContext2D, x: number, y: number, tile: TileData, light: LightSource) => {
      ...
      if (groupId) {
        ...
        if (matchingTiles.length > 0) {
          ...
          imgUrl = customTiles[group.tiles[0]];
          frameCount = tileMetadata[group.tiles[0]]?.frameCount || 1;
        }
      } else {
      ...
      const anchorX = x * size + size / 2;
      const anchorY = y * size - offsetY + imgH;
              ...
              if (tile) {
                ...
                if (hasShadow) {
                  ...
                  if (gid) {
                    ...
                    if (matchingTiles.length > 0) {
                      const index = (x * 7 + y * 13) % matchingTiles.length;
                      currentImgUrl = customTiles[matchingTiles[index]];
                    } else if (group.tiles.length > 0) {
                      currentImgUrl = customTiles[group.tiles[0]];
                    }
                  } else {
                  ...
                  const centerY = y * size - offsetY + currentImgH; 
